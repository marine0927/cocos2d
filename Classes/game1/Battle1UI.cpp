...
void Battle1UI::scheduleBlood(float delta)
{
	...
	heroprogress->setPercentage((((float)hero->getHp())*100.0 / hero->getInitHp()));  //这里是百分制显示
}
...
bool Battle1UI::init()
{
	...
	bossProgressTimer->setTag(50); 
	bossProgressTimer->setScale(1);//做一个标记
	this->addChild(bossProgressTimer);

	auto herobloodframe = Sprite::create("herobloodframe.png");
	auto herohead = Sprite::create("headshot.png");//创建进度框
	herobloodframe->setPosition(herobloodframe->getContentSize().width / 2+herohead->getContentSize().width, visibleSize.height - herobloodframe->getContentSize().height); //设置框的位置
	
	herohead->setPosition(herohead->getContentSize().width-12, visibleSize.height - herobloodframe->getContentSize().height); //设置框的位置
	this->addChild(herobloodframe); 
	this->addChild(herohead);//加到默认图层里面去
	auto herobloodbar = Sprite::create("herobloodbar.png");  //创建血条
	...
	heroProgressTimer->setPosition(herobloodframe->getContentSize().width/2 + herohead->getContentSize().width, visibleSize.height - herobloodframe->getContentSize().height);
	//从右到左减少血量
	...
	this->schedule(schedule_selector(Battle1UI::scheduleBlood),0.1f);  //刷新函数，每隔0.1秒
