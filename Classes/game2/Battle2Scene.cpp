...
#include "Battle2Scene.h"

USING_NS_CC;

Scene* Battle2::createScene()
{
	auto scene = Scene::createWithPhysics();
	scene->getPhysicsWorld()->setDebugDrawMask(PhysicsWorld::DEBUGDRAW_ALL);
	scene->getPhysicsWorld()->setGravity(Vec2(0, 0));
	auto layer = Battle2::create();
	auto battleUI = Battle2UI::create();
	scene->addChild(layer, 0, 0);
	scene->addChild(battleUI, 1,1);
	return scene;
}

void Battle2::scheduleSkill(cocos2d::CCObject * obj)
{
...
void Battle2::skillCoolDown(CCNode * node)
{
	//// 进度条技能不可见
...
bool Battle2::init()
{
	...
	tileMap = Sprite::create("map/Forest.png");
	if (tileMap == nullptr)
	{
		problemLoading("map/Forest.png");
	}
	else
	{
...
void Battle2::keyPressed(cocos2d::EventKeyboard::KeyCode keyCode, cocos2d::Event * event)
{
	Texture2D* herofront = Director::getInstance()->getTextureCache()->addImage("hero1.png");
...
void Battle2::keyReleased(cocos2d::EventKeyboard::KeyCode keyCode, cocos2d::Event * event)
{
...
void Battle2::mouseDown(cocos2d::EventMouse * event)
{
...
void Battle2::mouseMove(cocos2d::EventMouse * event)
{
	MousePoint.x = event->getCursorX();
	MousePoint.y = event->getCursorY();
}
void Battle2::update(float dt)
{
	Vec2 heroPos = hero->getPosition();
...
void Battle2::setViewpointCenter(Vec2 position)
{
...
void Battle2::onEnter()
{
	Layer::onEnter();
	auto contactlistener = EventListenerPhysicsContact::create();
	contactlistener->onContactBegin= CC_CALLBACK_1(Battle2::ContactBegin, this);
	contactlistener->onContactPreSolve = CC_CALLBACK_2(Battle2::ContactPreSolve, this);
	contactlistener->onContactPostSolve = CC_CALLBACK_2(Battle2::ContactPostSolve, this);
	contactlistener->onContactSeparate = CC_CALLBACK_1(Battle2::ContactSeparate, this);

	auto keyboardListener = EventListenerKeyboard::create();
	keyboardListener->onKeyPressed = CC_CALLBACK_2(Battle2::keyPressed, this);
	keyboardListener->onKeyReleased = CC_CALLBACK_2(Battle2::keyReleased, this);

	auto mouseListener = EventListenerMouse::create();
	mouseListener->onMouseDown = CC_CALLBACK_1(Battle2::mouseDown, this);
	mouseListener->onMouseMove = CC_CALLBACK_1(Battle2::mouseMove, this);
	...
	this->schedule(schedule_selector(Battle2::update), 1.0f / 60);
	this->schedule(schedule_selector(Battle2::BloodControl), 12.0f / 60);
	this->schedule(schedule_selector(Battle2::BossAttack), 90.0f / 60);
	this->schedule(schedule_selector(Battle2::traceSkill), 1.0f / 60);
}
bool Battle2::ContactBegin(PhysicsContact& contact)
{
...
bool Battle2::ContactPreSolve(PhysicsContact & contact, PhysicsContactPreSolve & solve)
{
	return true;
}
void Battle2::ContactPostSolve(PhysicsContact & contact, const PhysicsContactPostSolve & solve)
{

}
void Battle2::ContactSeparate(PhysicsContact & contact)
{

}
void Battle2::BloodControl(float dt)
{
	for (int i = 0; i <= 20; i++)
	{
		if (skillDemage[i] > 0) {
...
void Battle2::BossAttack(float dt)
{
	if (!bossAttack) {
		bossAttack = 1;
	}
...
void Battle2::setheroframe()
{

	
}

void Battle2::gotostore(Ref *pSender)
{
	auto sc = Store2::createScene();
	Director::getInstance()->replaceScene(sc);
}

void Battle2::traceSkill(float dt)
{
